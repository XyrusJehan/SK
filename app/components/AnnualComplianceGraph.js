...
export default function AnnualComplianceGraph() {
  ...
  // an assumed "current year + N past years" window.
  useEffect(() => {
  ...
  return (
    <View style={graphStyles.container}>
      ...
      <ScrollView
        ...
        style={graphStyles.yearScroll}
        contentContainerStyle={graphStyles.yearPicker}
      >
        {yearFilters.map((y) => {
          ...
          return (
            <TouchableOpacity
              key={y}
              style={[graphStyles.yearOption, active && graphStyles.yearOptionActive]}
              onPress={() => setSelectedYear(y)}
              activeOpacity={0.75}
            >
              <Text style={[graphStyles.yearOptionText, active && graphStyles.yearOptionTextActive]}>{y}</Text>
            </TouchableOpacity>
          );
        })}
      </ScrollView>
